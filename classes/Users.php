...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
  ...
  <strong>Error !</strong> Something went Wrong !</div>';
    ...
    // Add New Multi Users By Admin
    public function addNewMultiUsersByAdmin($name, $username, $email, $mobile, $roleid,$password, $count){
...
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Input fields must not be Empty (In user count '.$count.')!</div>';
              return $msg;
          } elseif (strlen($username) < 3) {
              $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Username is too short, at least 3 Characters (In user count '.$count.')!</div>';
              return $msg;
          } elseif (filter_var($mobile, FILTER_SANITIZE_NUMBER_INT) == FALSE) {
              $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Enter only Number Characters for Mobile number field (In user count '.$count.')!</div>';
              return $msg;

          } elseif (strlen($password) < 5) {
              $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Password at least 6 Characters (In user count '.$count.')!</div>';
              return $msg;
          } elseif (!preg_match("#[0-9]+#", $password)) {
              $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Your Password Must Contain At Least 1 Number (In user count '.$count.')!</div>';
              return $msg;
          } elseif (!preg_match("#[a-z]+#", $password)) {
              $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Your Password Must Contain At Least 1 Letter (In user count '.$count.')!</div>';
              return $msg;
          } elseif (filter_var($email, FILTER_VALIDATE_EMAIL === FALSE)) {
              $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Invalid email address (In user count '.$count.')!</div>';
              return $msg;
          } elseif ($checkEmail == TRUE) {
              $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Email already Exists, please try another Email... (In user count '.$count.')!</div>';
              return $msg;
          } else {
              ...
              $result = $stmt->execute();
              if ($result) {
                  $msg .= '<div class="alert alert-success alert-dismissible mt-3">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>Success !</strong> Wow, you have Registered Successfully (For user count '.$count.') !</div>';
                  return $msg;
              } else {
                  $msg .= '<div class="alert alert-danger alert-dismissible mt-3">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>Error !</strong> Something went Wrong (In user count '.$count.')!</div>';
                  return $msg;
              }
  ...
  public function userLoginAutho($email, $password){
    ...
    public function userLoginAuthoWithoutPass($email){
  ...
  <strong>Error !</strong> Email did not Found, use Register email or password please !</div>';
    ...
    public function loginWithGoogle($email, $data){
        ...
        $logResult = $this->userLoginAuthoWithoutPass($email);
        $chkActive = $this->CheckActiveUser($email);
    ...
    <strong>Error !</strong> Sorry, Your account is Diactivated, Contact with Admin !</div>';
            ...
            $sql = "INSERT INTO tbl_users(name, username, email, roleid) VALUES(:name, :username, :email, :roleid)";
            $stmt = $this->db->pdo->prepare($sql);
            $stmt->bindValue(':name', $name);
            $stmt->bindValue(':username', $username);
            $stmt->bindValue(':email', $email);
            ...
            if ($result) {
                ...
                $logResult2 = $this->userLoginAuthoWithoutPass($email);

                Session::init();
                Session::set('login', TRUE);
                Session::set('id', $logResult2->id);
                Session::set('roleid', $logResult2->roleid);
                Session::set('name', $logResult2->name);
                Session::set('email', $logResult2->email);
                Session::set('username', $logResult2->username);
                Session::set('logMsg', '<div class="alert alert-success alert-dismissible mt-3" id="flash-msg">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <strong>Success !</strong> You are Logged In Successfully !</div>');
                echo "<script>location.href='index.php';</script>";
